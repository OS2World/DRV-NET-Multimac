...
static inline void iwl_free_fw_desc(adapter,
				    struct fw_desc *desc)
{
	if (desc->v_addr)
		dma_free_coherent(adapter->pci_dev_id, desc->len,
				  desc->v_addr, desc->p_addr);
	desc->v_addr = NULL;
	desc->len = 0;
}

static inline int iwl_alloc_fw_desc(adapter *,
				    struct fw_desc *desc)
{
	if (!desc->len) {
		desc->v_addr = NULL;
	...
	desc->v_addr = dma_alloc_coherent(pci_dev_id, desc->len,
